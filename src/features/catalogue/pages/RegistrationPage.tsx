  ...
  if (step === 0) {
    return (
      <>
        ...
        <div className="split-row verify-row"><label>OFFICIAL EMAIL*<input value={registrationForm.email} onChange={(e) => updateRegistrationField('email', e.target.value)} placeholder="admin@yourorg.com" /></label><button type="button" className="verify-button">Verify</button></div>
        <div className="split-row verify-row"><label>OFFICIAL PHONE NUMBER*<input value={registrationForm.phone} onChange={(e) => updateRegistrationField('phone', e.target.value)} placeholder="+91-XXXXXXXXXX" /></label><button type="button" className="verify-button">Verify</button></div>
      </>
    )
  }
  if (step === 1) return <><div className="split-row verify-row"><label>REGISTRATION NUMBER*<input value={registrationForm.gst} onChange={(e) => updateRegistrationField('gst', e.target.value)} placeholder="GST / CIN / NGO registration number" /></label><button type="button" className="verify-button">Verify</button></div></>
  if (step === 2) return <><div className="step-row"><label>REPRESENTATIVE NAME*<input value={registrationForm.repName} onChange={(e) => updateRegistrationField('repName', e.target.value)} placeholder="Enter representative name" /></label></div><div className="split-row verify-row"><label>REPRESENTATIVE EMAIL*<input value={registrationForm.repEmail} onChange={(e) => updateRegistrationField('repEmail', e.target.value)} placeholder="rep@yourorg.com" /></label><button type="button" className="verify-button">Verify</button></div><div className="step-row"><label>MOBILE NUMBER<input value={registrationForm.repMobile} onChange={(e) => updateRegistrationField('repMobile', e.target.value)} placeholder="+91-XXXXXXXXXX" /></label></div><div className="step-row"><label>DESIGNATION<input value={registrationForm.designation} onChange={(e) => updateRegistrationField('designation', e.target.value)} placeholder="Designation" /></label></div><div className="step-row"><label>EMPLOYEE ID<input value={registrationForm.empId} onChange={(e) => updateRegistrationField('empId', e.target.value)} placeholder="Employee ID" /></label></div></>
